...
class DeformConv2d_v1(nn.Module):
    def __init__(
            self,
            in_channels: int,
            out_channels: int,
        ...
        super(DeformConv2d_v1, self).__init__()
        ...
        self.p_conv.weight.data = torch.zeros_like(self.p_conv.weight.data)
        if self.p_conv.bias is not None:
            self.p_conv.bias.data = torch.zeros_like(self.p_conv.bias.data)
